...
## 1a. Vendoring OCR models (optional, for offline / privacy)

The OCR feature (`src/features/ocr/ocrProcessor.ts`) uses **tesseract.js**, which by
default downloads its worker, wasm core, and language models from a CDN
(`cdn.jsdelivr.net` + `tessdata.projectnaptha.com`) on first use. Vendoring these
locally keeps everything on-device — better for privacy, offline use, and supply-chain
safety.
...
| --- | --- | --- |
| Worker script (`worker.min.js`) | `node_modules/tesseract.js/dist` | `public/tesseract/` |
| Wasm core (`tesseract-core*.wasm.js` + `.wasm`) | `node_modules/tesseract.js-core` | `public/tesseract/` |
| Language data (`eng.traineddata.gz`, `bul.traineddata.gz`) | CDN (`tessdata.projectnaptha.com`) | `public/tesseract/lang/` |

**Worker + core are copied automatically** — `scripts/copy-tesseract-assets.mjs` runs
via the `predev` / `prebuild` npm hooks (i.e. before `npm run dev` and `npm run build`),
so no manual step is needed. To run it on demand:

```bash
npm run vendor:ocr-core       # copies worker.min.js + tesseract-core* into public/tesseract/
```

**Language traineddata must be fetched manually** (it isn't shipped in node_modules).
Run the fetch script to download the models into `public/tesseract/lang/`:

```bash
npm run vendor:ocr-lang       # or: bash scripts/fetch-tesseract-lang.sh
```

This curls `eng.traineddata.gz` and `bul.traineddata.gz` from
`https://tessdata.projectnaptha.com/4.0.0/`. The UI supports English (`eng`),
Bulgarian (`bul`), and both (`eng+bul`).

**Without vendoring the language data, OCR still works** — `ocrProcessor.ts` tries the
local `/tesseract` paths first and, if the worker fails to initialize (assets missing),
falls back once to tesseract.js's default CDN paths (logging a single `console.warn`).
The `vercel.json` CSP intentionally keeps `cdn.jsdelivr.net` and
`tessdata.projectnaptha.com` in `connect-src` to allow this fallback. Once models are
vendored, the local path is used first and no CDN request is made.

> The vendored binaries under `public/tesseract/` are git-ignored (see
> `public/tesseract/.gitignore`) — they are regenerated by the scripts above.
