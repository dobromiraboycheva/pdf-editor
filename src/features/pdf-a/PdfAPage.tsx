...
export function PdfAPage() {
  ...
  const handleConvert = useCallback(async () => {
    ...
    try {
      ...
      if (ac.signal.aborted || (e as Error).message === 'aborted') {
        ...
        toast({ message: t('tools.pdfA.failed', { message: (e as Error).message }), variant: 'error' });
      }
    } finally {
      setBusy(false);
  ...
  return (
    <ToolShell
      title={t('tools.pdfA.name')}
      tagline={t('tools.pdfA.description')}
      onStartOver={file ? clear : undefined}
      sidebar={
        <div className="flex flex-col gap-4">
          <p className="text-xs text-ink-muted">{t('tools.pdfA.note')}</p>
          <ProcessButton
            label={t('tools.pdfA.cta')}
          ...
          {result && (
            <div className="rounded-lg bg-emerald-50 p-3 text-sm text-emerald-900">
              {t('tools.pdfA.success', { size: formatBytes(result.size) })}
            </div>
          )}
    ...
    >
      ...
      <ProgressOverlay
        open={busy}
        label={note || t('tools.pdfA.progress')}
        fraction={progress}
