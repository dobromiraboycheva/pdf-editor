...
export function PdfToPowerpointPage() {
  ...
  const handleConvert = useCallback(async () => {
    ...
    abortRef.current = ac;
    try {
      const res = await pdfToPowerpointProcessor({
        files: [file],
        options: {},
        signal: ac.signal,
        onProgress: (f, n) => {
      ...
      if (ac.signal.aborted || (e as Error).message === 'aborted') {
        ...
        toast({ message: t('tools.pdfToPowerpoint.failed', { message: (e as Error).message }), variant: 'error' });
      }
    } finally {
      setBusy(false);
  ...
  return (
    <ToolShell
      title={t('tools.pdfToPowerpoint.name')}
      tagline={t('tools.pdfToPowerpoint.description')}
      onStartOver={file ? clear : undefined}
      sidebar={
        <div className="flex flex-col gap-4">
          <ProcessButton
            label={t('tools.pdfToPowerpoint.cta')}
            onClick={handleConvert}
            disabled={!canConvert}
            loading={busy}
          />
          <p className="text-xs text-ink-muted">
            {t('tools.pdfToPowerpoint.noteSlideshow')}
          </p>
          {!file && (
            <p className="text-xs text-ink-muted">
              {t('tools.pdfToPowerpoint.hintEmpty')}
            </p>
          )}
          {result && (
            <div className="rounded-lg bg-emerald-50 p-3 text-sm text-emerald-900">
              {t('tools.pdfToPowerpoint.success', {
                name: result.name,
    ...
    >
      ...
      <ProgressOverlay
        open={busy}
        label={note || t('tools.pdfToPowerpoint.progress')}
        fraction={progress}
